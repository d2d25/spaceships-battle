...
$sql = 'SELECT idVaisseau FROM joueurs_vaisseaux WHERE possede = 1';
$stmt = $pdo->prepare($sql);
$stmt->execute();
$vaisseauxpossedes2 = $stmt->fetchAll(PDO::FETCH_ASSOC);

$sql = 'SELECT idVaisseau FROM joueurs_vaisseaux WHERE disponibleAchat = 0';
$stmt = $pdo->prepare($sql);
$stmt->execute();
$vaisseauxnonDisponibleAchat = $stmt->fetchAll(PDO::FETCH_ASSOC);
// dump('',$vaisseaux);die;
...
foreach ($vaisseaux as $vaisseau) {
    ...
    if (array_key_exists($vaisseau['idVaisseau'], $vaisseauxpossedes2)) {
        $vaisseauxPossedes[] = $vaisseau['idVaisseau'];
    }
    if (array_key_exists($vaisseau['idVaisseau'], $vaisseauxnonDisponibleAchat)) {
    ...
    if (!empty($_POST['vaisseau_choisi'])) {
        ...
        if (!in_array($vaisseauChoisi, $vaisseauxPossedes)) {
            ...
            if ($prixVaisseauChoisi <= $argentJoueur) {
                ...
                $un = 1;
                $zero = 0;
                $sql = 'INSERT INTO joueurs_vaisseaux (idJoueur,idVaisseau, possede, disponibleAchat) VALUES (:1,:2,:3,:4)';
                $stmt = $pdo->prepare($sql);
